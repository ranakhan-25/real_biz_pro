...
const initialRoads: RoadItem[] = [
  { sl: 1, projectName: 'Estern 19', roadNo: '3/A' },
  { sl: 2, projectName: 'Estern 19', roadNo: '2/A' },
  { sl: 3, projectName: 'N/A', roadNo: '7' },
  { sl: 4, projectName: 'N/A', roadNo: 'Road 10' },
  { sl: 5, projectName: 'N/A', roadNo: '7' },
  { sl: 6, projectName: 'N/A', roadNo: '2' },
  { sl: 7, projectName: 'N/A', roadNo: '1' },
  { sl: 8, projectName: 'Green Valley', roadNo: 'Main Avenue' },
  { sl: 9, projectName: 'Lake Garden', roadNo: 'Road 4/B' },
  { sl: 10, projectName: 'Sheba Tower', roadNo: 'Road 12' },
];

export default function PropertyRoadListPage() {
  const [roads, setRoads] = useState<RoadItem[]>(initialRoads);
  ...
  const handleOpenAdd = () => {
    setEditingRoad(null);
    setFormData({ projectName: '', roadNo: '' });
    setIsModalOpen(true);
  };
  ...
  const handleOpenEdit = (road: RoadItem) => {
    setEditingRoad(road);
    setFormData({ projectName: road.projectName, roadNo: road.roadNo });
    setIsModalOpen(true);
  };
  ...
  const handleDelete = (sl: number) => {
    if (confirm('Are you sure you want to delete this road?')) {
      setRoads(roads.filter(item => item.sl !== sl));
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingRoad) {
      // Update
      setRoads(roads.map(item => item.sl === editingRoad.sl ? { ...item, ...formData } : item));
    } else {
      // Add New
      ...
      setRoads([newItem, ...roads]);
    }
    setIsModalOpen(false);
  };

  // Filter & Search Logic
  const filteredRoads = roads.filter(item => {
    const matchesSearch = item.projectName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          item.roadNo.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="min-h-screen bg-slate-50/65 text-slate-800 p-4 md:p-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-2 text-sm text-slate-500">
          ...
          <ChevronRight className="w-4 h-4 text-slate-400" />
          <span className="text-indigo-600 font-semibold">Property Road List</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Plus className="w-4 h-4" /> Road Add
          </button>
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 mb-6">
        <div className="flex items-center gap-2 mb-4 text-slate-700 font-semibold text-sm">
          ...
          <span>Filter Roads by Project</span>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-6">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gradient-to-r from-indigo-600 via-indigo-700 to-violet-700 text-white text-xs uppercase tracking-wider font-semibold">
                ...
                <th className="py-3.5 px-4">Road No</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
              {filteredRoads.length > 0 ? (
                filteredRoads.map((item, index) => (
                  <tr key={index} className="hover:bg-slate-50/80 transition group">
                    ...
                    <td className="py-3.5 px-4 font-medium text-indigo-600 flex items-center gap-1.5">
                      <MapPin className="w-3.5 h-3.5 text-indigo-400" /> {item.roadNo}
                    </td>
                ...
                <tr>
                  <td colSpan={4} className="text-center py-10 text-slate-400">No roads found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between p-4 bg-slate-50/50 border-t border-slate-100 text-xs text-slate-500">
          <div>Showing 1 to {filteredRoads.length} of {roads.length} entries</div>
      ...
      {/* FORM 2: Road Add / Edit Modal Form */}
      {isModalOpen && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-indigo-600 to-violet-600 text-white">
              <h3 className="font-semibold text-base flex items-center gap-2">
                <Plus className="w-5 h-5" /> {editingRoad ? 'Edit Road' : 'Add New Road'}
              </h3>
              <button 
