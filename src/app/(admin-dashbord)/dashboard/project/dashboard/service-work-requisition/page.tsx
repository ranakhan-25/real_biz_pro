...
const initialData: ServiceRequisition[] = [
  {
    id: 1,
    ...
    grandTotal: 45000,
    addedBy: "Admin",
    approvalLayer: [{ label: "All Approvals Completed", done: true }],
  },
  {
    id: 2,
    projectType: "Office",
    ...
    addedBy: "Tazmul Reza",
    approvalLayer: [
      { label: "All Approvals Completed", done: true },
      { label: "Rifat Hosain", done: true },
...
const ServiceWorkRequisitionPage = () => {
  const [rows, setRows] = useState<ServiceRequisition[]>(initialData);
  ...
  const openEditModal = (row: ServiceRequisition) => {
    setEditingId(row.id);
    setForm({
    ...
    <div className="p-6">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span>›</span>
          <span className="text-slate-600">Service/Work Requisition List</span>
        </div>
        ...
        >
          <Plus className="h-4 w-4" /> New Service/Work Requisition
        </button>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between text-sm">
        <span>
          Showing {filtered.length === 0 ? 0 : 1} to{" "}
          {Math.min(entries, filtered.length)} of {filtered.length} entries
        </span>
          ...
          <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              <h2 className="text-lg font-bold text-slate-900">
                {editingId !== null ? "Edit Service/Work Requisition" : "New Service/Work Requisition"}
              </h2>
              <button
                onClick={closeModal}
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 px-6 py-5 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label className="mb-1 block text-sm font-medium text-slate-600">Grand Total</label>
                <input
                  type="number"
...
export default ServiceWorkRequisitionPage;