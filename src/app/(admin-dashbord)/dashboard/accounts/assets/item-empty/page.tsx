...
export default function AssetManagementSystem() {
  ...
  const [locationList, setLocationList] = useState<string[]>(['Head Office', 'Branch Office', 'Warehouse A']);

  // History states for the active asset details
  const [movements, setMovements] = useState<MovementItem[]>([
    { date: '01 Dec 2025', fromLocation: 'Warehouse', toLocation: 'Head Office', reason: 'Initial Setup' }
  ]);
  const [revaluations, setRevaluations] = useState<RevaluationItem[]>([]);

  // Modal State for dynamic additions (+)
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [modalType, setModalType] = useState<'item' | 'project' | 'movement' | 'depreciation' | 'revaluation'>('item');
  const [dynamicInputVal, setDynamicInputVal] = useState('');
  
  // Extra modal form states
  const [movToLoc, setMovToLoc] = useState('Head Office');
  const [movReason, setMovReason] = useState('');
  const [revalNewVal, setRevalNewVal] = useState<number | ''>('');
  const [revalReason, setRevalReason] = useState('');
  const [newDepMethod, setNewDepMethod] = useState('Straight Line');
  ...
  const handleModalSubmit = (e: React.FormEvent) => {
    ...
    if (modalType === 'item') {
      if (!dynamicInputVal.trim()) return alert('Please enter item name');
      setItemList([...itemList, dynamicInputVal]);
      setItemName(dynamicInputVal);
      alert(`New Item "${dynamicInputVal}" added!`);
    } else if (modalType === 'project') {
      if (!dynamicInputVal.trim()) return alert('Please enter project name');
      setProjectList([...projectList, dynamicInputVal]);
      setProject(dynamicInputVal);
      alert(`New Project "${dynamicInputVal}" added!`);
    } else if (modalType === 'movement') {
      if (!movReason.trim()) return alert('Please enter movement reason');
    ...
    } else if (modalType === 'depreciation') {
      setMethod(newDepMethod);
      alert(`Depreciation method updated to ${newDepMethod}!`);
    } else if (modalType === 'revaluation') {
      if (revalNewVal === '' || typeof originalValue !== 'number') return alert('Invalid value');
      const newRev: RevaluationItem = {
        date: new Date().toLocaleDateString(),
        previousValue: originalValue,
        newValue: Number(revalNewVal),
        reason: revalReason || 'Market value update'
      };
      setRevaluations([newRev, ...revaluations]);
      setOriginalValue(Number(revalNewVal));
      alert('Asset revaluation recorded successfully!');
    }
  ...
  // Calculate Depreciation Schedule Rows
  const renderDepreciationBoard = () => {
    const years = parseInt(duration) || 1;
    const val = typeof originalValue === 'number' ? originalValue : 0;
    const annualDep = val / years;
    let currentBook = val;

    const rows = [];
    for (let i = 1; i <= years; i++) {
      const beginningBook = currentBook;
      const endingBook = Math.max(0, currentBook - annualDep);
      currentBook = endingBook;

      rows.push(
        <tr key={i} className="border-b border-slate-200 hover:bg-slate-50">
          <td className="p-3 font-medium">Year {i}</td>
          <td className="p-3">{beginningBook.toFixed(2)}</td>
          <td className="p-3 text-rose-600 font-semibold">{annualDep.toFixed(2)}</td>
          <td className="p-3 font-semibold text-slate-900">{endingBook.toFixed(2)}</td>
        </tr>
      );
    }
    return rows;
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-6 font-sans text-slate-800 relative">
      ...
      {/* ================= VIEW 2: ASSET DETAILS & TABS PAGE ================= */}
      {activeView === 'details' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button 
                onClick={() => { setModalType('revaluation'); setIsModalOpen(true); }} 
                className="bg-[#6b58e8] hover:bg-[#5844d0] text-white text-xs font-semibold px-3.5 py-2 rounded-lg shadow transition flex items-center gap-1"
              >
                <Plus className="w-3.5 h-3.5" /> Revaluation
              </button>
            </div>
          </div>

          <div className="bg-white rounded-xl shadow-md border border-slate-200 px-6 py-2 flex space-x-8 text-sm font-medium">
            {(['Asset', 'Depreciation Board', 'Movement History', 'Revaluations History'] as const).map((tab) => (
              <button
                key={tab}
          ...
          {activeTab === 'Asset' && (
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8 space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="pt-6 space-y-4">
                    ...
                    <div className="space-y-4 text-sm">
                      ...
                      <div>
                        <label className="block text-slate-700 font-medium mb-1.5">Voucher No</label>
                        <select 
                          value={voucherNo} 
              ...
              <div className="flex items-center space-x-4 pt-6 border-t border-slate-200">
                <button onClick={handleSaveAsset} className="bg-[#6b58e8] hover:bg-[#5844d0] text-white px-6 py-2.5 rounded-lg text-sm font-semibold shadow transition">
                  Save
                </button>
                <button onClick={() => setActiveView('list')} className="bg-slate-200 hover:bg-slate-300 text-slate-700 px-6 py-2.5 rounded-lg text-sm font-semibold transition">
                  Cancel
                </button>
              </div>
            </div>
          ...
          {/* TAB 2: DEPRECIATION BOARD */}
          {activeTab === 'Depreciation Board' && (
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8 space-y-6">
              <h3 className="text-base font-bold text-slate-900">Calculated Depreciation Schedule</h3>
              <div className="overflow-x-auto border border-purple-300 rounded-xl">
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="bg-[#6b58e8] text-white font-bold">
                      <th className="p-3">Period</th>
                      <th className="p-3">Beginning Book Value</th>
                      <th className="p-3">Depreciation Amount</th>
                      <th className="p-3">Ending Book Value</th>
                    </tr>
                  </thead>
                  <tbody>
                    {renderDepreciationBoard()}
                  </tbody>
                </table>
              </div>
            </div>
          ...
          {activeTab === 'Movement History' && (
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8 space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="text-base font-bold text-slate-900">Asset Transfer & Movement Log</h3>
                <button onClick={() => { setModalType('movement'); setIsModalOpen(true); }} className="bg-[#6b58e8] text-white text-xs px-3.5 py-2 rounded-lg">
                  + Add Movement
                </button>
              </div>
              <div className="overflow-x-auto border border-slate-200 rounded-xl">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {movements.length === 0 ? (
                      <tr><td colSpan={4} className="p-6 text-center text-slate-400">No movement history recorded.</td></tr>
                    ) : (
                      movements.map((m, i) => (
                        <tr key={i} className="border-b border-slate-100">
                          <td className="p-3">{m.date}</td>
                          <td className="p-3">{m.fromLocation}</td>
                          <td className="p-3 font-semibold text-purple-700">{m.toLocation}</td>
          ...
          {/* TAB 4: REVALUATIONS HISTORY */}
          {activeTab === 'Revaluations History' && (
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8 space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="text-base font-bold text-slate-900">Asset Revaluation History</h3>
                <button onClick={() => { setModalType('revaluation'); setIsModalOpen(true); }} className="bg-[#6b58e8] text-white text-xs px-3.5 py-2 rounded-lg">
                  + New Revaluation
                </button>
              </div>
              <div className="overflow-x-auto border border-slate-200 rounded-xl">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {revaluations.length === 0 ? (
                      <tr><td colSpan={4} className="p-6 text-center text-slate-400">No revaluation records available.</td></tr>
                    ) : (
                      revaluations.map((r, i) => (
                        <tr key={i} className="border-b border-slate-100">
                          <td className="p-3">{r.date}</td>
                          <td className="p-3">{r.previousValue}</td>
                          <td className="p-3 font-semibold text-emerald-600">{r.newValue}</td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50">
              <h3 className="text-base font-bold text-slate-900 capitalize">
                ...
                {modalType === 'movement' && 'Record Asset Movement'}
                {modalType === 'depreciation' && 'Modify Depreciation Method'}
                {modalType === 'revaluation' && 'Record Asset Revaluation'}
              </h3>
            ...
            {(modalType === 'item' || modalType === 'project') && (
              <form onSubmit={handleModalSubmit} className="p-6 space-y-4">
                ...
                <div className="flex justify-end space-x-2 pt-2">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="bg-slate-200 text-slate-700 px-4 py-2 rounded-lg text-sm font-medium">Cancel</button>
                  <button type="submit" className="bg-[#6b58e8] text-white px-4 py-2 rounded-lg text-sm font-semibold shadow">Add</button>
                </div>
              </form>
            )}
            ...
            {/* Movement Modal Form */}
            {modalType === 'movement' && (
              <form onSubmit={handleModalSubmit} className="p-6 space-y-4 text-sm">
                <div>
                  <label className="block font-medium mb-1">Transfer to Location</label>
                  <select value={movToLoc} onChange={(e) => setMovToLoc(e.target.value)} className="w-full border rounded-lg px-3 py-2">
                    {locationList.map((l, idx) => <option key={idx} value={l}>{l}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block font-medium mb-1">Reason / Note</label>
                  <input type="text" value={movReason} onChange={(e) => setMovReason(e.target.value)} placeholder="e.g. Branch relocation" className="w-full border rounded-lg px-3 py-2" required />
                </div>
                <div className="flex justify-end space-x-2 pt-2">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="bg-slate-200 px-4 py-2 rounded-lg">Cancel</button>
                  <button type="submit" className="bg-[#6b58e8] text-white px-4 py-2 rounded-lg shadow">Save Movement</button>
                </div>
              </form>
            )}

            {/* Depreciation Modification Form */}
            {modalType === 'depreciation' && (
              <form onSubmit={handleModalSubmit} className="p-6 space-y-4 text-sm">
                <div>
                  <label className="block font-medium mb-1">New Depreciation Method</label>
                  <select value={newDepMethod} onChange={(e) => setNewDepMethod(e.target.value)} className="w-full border rounded-lg px-3 py-2">
                    <option value="Straight Line">Straight Line</option>
                    <option value="Reducing Balance">Reducing Balance</option>
                  </select>
                </div>
                <div className="flex justify-end space-x-2 pt-2">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="bg-slate-200 px-4 py-2 rounded-lg">Cancel</button>
                  <button type="submit" className="bg-[#6b58e8] text-white px-4 py-2 rounded-lg shadow">Update Method</button>
                </div>
              </form>
            )}

            {/* Revaluation Form */}
            {modalType === 'revaluation' && (
              <form onSubmit={handleModalSubmit} className="p-6 space-y-4 text-sm">
                <div>
                  <label className="block font-medium mb-1">New Asset Valuation Amount</label>
                  <input type="number" value={revalNewVal} onChange={(e) => setRevalNewVal(e.target.value ? Number(e.target.value) : '')} placeholder="Enter updated value" className="w-full border rounded-lg px-3 py-2" required />
                </div>
                <div>
                  <label className="block font-medium mb-1">Reason for Revaluation</label>
                  <input type="text" value={revalReason} onChange={(e) => setRevalReason(e.target.value)} placeholder="e.g. Market price appreciation" className="w-full border rounded-lg px-3 py-2" />
                </div>
                <div className="flex justify-end space-x-2 pt-2">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="bg-slate-200 px-4 py-2 rounded-lg">Cancel</button>
                  <button type="submit" className="bg-[#6b58e8] text-white px-4 py-2 rounded-lg shadow">Save Revaluation</button>
                </div>
              </form>
            )}
