...
interface EquityRow {
  ...
  shareMoneyDeposits: number | string;
...
export default function StatementOfChangesInEquity() {
  ...
  // Statement of Changes in Equity Exact Data Structure matching the image
  const reportData: EquityRow[] = [
    { particulars: 'Opening balance', shareCapital: 0.00, shareMoneyDeposits: '-', retainedEarnings: '-', total: 0.00, type: 'bold' },
    { particulars: 'Share issue during the period', shareCapital: '-', shareMoneyDeposits: '-', retainedEarnings: '-', total: '-', type: 'normal' },
    { particulars: 'Share Money Deposit', shareCapital: '-', shareMoneyDeposits: '-', retainedEarnings: '-', total: '-', type: 'normal' },
    { particulars: 'Net Profit/(loss) after Tax', shareCapital: '-', shareMoneyDeposits: '-', retainedEarnings: '-', total: '-', type: 'normal' },
    { particulars: 'Closing balance (01.01.1970)', shareCapital: 0.00, shareMoneyDeposits: '-', retainedEarnings: '-', total: 0.00, type: 'bold' },
  ];
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center text-sm text-slate-600 space-x-2">
            ...
            <ChevronRight className="w-4 h-4 text-slate-400" />
            <span className="font-semibold text-slate-900">Statement of Changes in Equity</span>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6 space-y-6">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="flex items-center space-x-2 pb-0.5">
              <button 
                onClick={() => alert('Exporting Statement of Changes in Equity to Excel...')}
              ...
              <button 
                onClick={() => alert('Generating Statement of Changes in Equity PDF...')}
            ...
            <table className="w-full text-left border-collapse text-xs sm:text-sm">
              <thead>
                <tr className="bg-[#6b58e8] text-white font-bold tracking-wide text-center">
                  <th className="p-3.5 border-r border-purple-400 text-left">PARTICULARS</th>
                  <th className="p-3.5 border-r border-purple-400 w-44">SHARE CAPITAL<br /><span className="text-[10px] font-normal">BDT</span></th>
                  <th className="p-3.5 border-r border-purple-400 w-48">SHARE MONEY DEPOSITS<br /><span className="text-[10px] font-normal">BDT</span></th>
                  <th className="p-3.5 border-r border-purple-400 w-44">RETAINED EARNINGS<br /><span className="text-[10px] font-normal">BDT</span></th>
                  <th className="p-3.5 w-32">TOTAL</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr 
                      ...
                      className={`border-b border-slate-200 transition-colors ${
                        isBold ? 'bg-slate-50 font-bold text-slate-900' : 'hover:bg-slate-50'
                      }`}
                    >
                      {/* Particulars */}
                      ...
                      {/* Share Capital */}
                      <td className="p-3.5 border-r border-slate-200 text-center font-medium">
                        {typeof row.shareCapital === 'number' 
                          ? row.shareCapital.toLocaleString('en-US', { minimumFractionDigits: 2 }) 
                          : row.shareCapital}
                      </td>

                      {/* Share Money Deposits */}
                      <td className="p-3.5 border-r border-slate-200 text-center font-medium">
                        {row.shareMoneyDeposits}
                      </td>
